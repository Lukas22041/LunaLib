...
internal class LunaDebugUIEntitiesPanel : lunalib.backend.ui.debug.LunaDebugUIInterface {

    var parentPanel: CustomPanelAPI? = null
    var parentClass: lunalib.backend.ui.debug.LunaDebugUIMainPanel? = null
   ...
   // var filters = listOf("Current System", "Starsystems", "Stars", "Planets" ,"Custom Entities", "Fleets")
    ...
    override fun init(parentPanel: CustomPanelAPI, parentClass: lunalib.backend.ui.debug.LunaDebugUIMainPanel, panel: CustomPanelAPI) {
        ...
        var searchField = LunaUITextField("",0f, 0f, 250f, 30f,"Empty", "Debug", panel, panelElement!!).apply {
            onUpdate {
                ...
                if (field.paragraph != null && isSelected() && lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.searchText != field.paragraph!!.text)
                {
                    lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.searchText = field!!.paragraph!!.text
                    searchForEntities()
                    createList()
                }
            }
        }
        searchField.paragraph!!.text = lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.searchText
        searchField.value = lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.searchText
        ...
        if (lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.searchText == "")
        {
            para.text = "Search"
        }
        ...
        for ((key, value) in lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.filters)
        {
            ...
            button.onClick {
                ...
                lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.filters.put(key, button.value)
        ...
        for (entity in entities)
        {
           ...
           // cardPanel.uiElement.addComponent(interactbleElement)
            ...
            else if (entity is SectorEntityToken)
            {
                ...
                {
                    var sprite = lunalib.backend.ui.components.base.LunaUISprite(entity.customEntitySpec.spriteName,
                        80f,
                        80f,
                        40f,
                        40f,
                ...
                {
                    var sprite = lunalib.backend.ui.components.base.LunaUISprite(entity.customEntitySpec.iconName,
                        80f,
                        80f,
                        40f,
                        40f,
    ...
    fun searchForEntities()
    {
        ...
        var curSystemButton = lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.filters.get("Current System Only")
        if (lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.filters.get("Current System Only")!!)
        {
        ...
        var searchInput = lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.searchText.lowercase()


        //cant use fuzzy string matching, as this goes through way to many entities and starts becoming laggy quickly.
        if (lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.filters.get("Starsystems")!!)
        {
            run search@ {
                systems.forEach {
                    if (currentSystem && it != system) return@forEach
                    if (lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.searchText.isEmpty()) { entities.add(it); capCount++ }
        ...
        if (lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.filters.get("Stars")!!)
        {
            run search@ {
                planets.forEach {
        ...
        if (lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.filters.get("Planets")!!)
        {
            run search@ {
                planets.forEach {
                    if (currentSystem && it.starSystem != system) return@forEach
        ...
        if (lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.filters.get("Custom Entities")!!)
        {
            run search@ {
        ...
        if (lunalib.backend.ui.debug.LunaDebugUIEntitiesPanel.Companion.filters.get("Fleets")!!)
        {
            run search@ {
                fleets.forEach {
                    if (currentSystem && it.starSystem != system) return@forEach
